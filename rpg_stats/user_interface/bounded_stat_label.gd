...
var bounded_stat: BoundedStat:
	set(p_stat):
		...
		if bounded_stat and bounded_stat.value_changed.is_connected(_set_label_text):
...
func _set_label_text(_p_ignored_value: int = 0) -> void:
	if bounded_stat:
		text = str(bounded_stat.max_stat.stat_name, ": ", bounded_stat.max_stat.value, "/", bounded_stat.current_value);
	else:
		text = "No Stat"
